...
let v = [...Array(4)].map(x => Array(3).fill(0))
let T = Array(3).fill(null);

const init = new Set().add('v'+String(Array(4).fill(0)));
for (let i = 0; i < 3; i++) init.add(i+String(s.map(l => l[i])));
...
for (let i = 0; T.includes(null); i++) {
	...
	for (let ax = 0; ax < 3; ax++) {
		if (T[ax] === null) {
		    if (init.has('v'+String(v.map(l => l[ax]))) && init.has(ax+String(s.map(l => l[ax]))))
			T[ax] = i+1;
		}
	}
}
